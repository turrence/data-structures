...
class HashTableQuadPr:
    ...
    def insert_stop_table(self, word):
        ...
        # print('original hash:', hashValue)
        i = 0
        #  # print('??? hash:', hashValue + (i ** 2))
        while self.hash_table[hashValue + (i ** 2)]:  # quadratic probing
            # assumes no repeat words
            # # print('collision at', hashValue + i ** 2, 'contains word:', self.hash_table[hashValue + i ** 2])
            # # print('\tfor word:', actualWord)
            # # print('old i:', i)
            # # print('old hash:', hashValue + i ** 2)
            i += 1
            # print('new i:', i)
            # print('new hash:', hashValue + i ** 2)
            if hashValue + i ** 2 >= len(self.hash_table):
                hashValue -= len(self.hash_table)
        self.hash_table[hashValue + i ** 2] = actualWord
        # print('inserted', actualWord, 'at', hashValue + i ** 2)
        self.num_items += 1
    ...
    def insert_word(self, word, line_num):
        ...
        i = 0
        while self.hash_table[hashValue + i ** 2] and self.hash_table[hashValue + i ** 2][0] != word:
            i += 1  # quadratic probing
            #print('collision at', hashValue + i ** 2, "with", self.hash_table[hashValue + i ** 2], 'for', word)
            if hashValue + i ** 2 >= len(self.hash_table):
                hashValue -= len(self.hash_table)
        if not self.hash_table[hashValue+ i ** 2] and type(line_num) is not list:
            self.hash_table[hashValue + i ** 2] = (word, [line_num])
            self.num_items += 1
        elif not self.hash_table[hashValue+ i ** 2] and type(line_num) is list:
            self.hash_table[hashValue + i ** 2] = (word, line_num)
            self.num_items += 1
        elif self.word_exists_in_line(hashValue + i ** 2, line_num):
            return
        elif self.hash_table[hashValue + i ** 2][0] == word:
            self.hash_table[hashValue + i ** 2][1].append(line_num)
        if self.get_load_fact() > 0.75:
            self.rehash()
    ...
    # see sample output file for format
    ...
    def __repr__(self):
        return 'HashTableQuadPr({})'.format(self.hash_table)